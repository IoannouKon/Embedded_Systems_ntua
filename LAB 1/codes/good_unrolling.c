                ...
                for(i=-S; i<S+1; i+=S) // 3 loops for S =-4
                {
                    ...
                    for(k=0; k<B; k++)
                    {
                        for(l=0; l<B; l++)
                        {
                            ...
                            ///X calculations#######################################################
                            if(( B*x + vectors_y[x][y] + i + k) < 0 ||
                                    ( B*x + vectors_y[x][y] + i + k) > (N-1) ||
                                    ( B*y + _vectors_y[x][y] + l) < 0 ||
                                    ( B*y + vectors_y[x][y] + l) > (M-1))

                            {
                                p2 = 0;
                            } else {
                                p2 = previous[  B*x + vectors_y[x][y]+i+k][ B*y + vectors_y[x][y]+l];
                            }
                            distx += abs(p1 - p2);
                            if (i !=0) { // Here we ADD IF condition
                                ///Y calculations######################################################
                                if((  B*x + vectors_y[x][y] + k) <0 ||
                                        (  B*x + vectors_y[x][y] + k) > (N-1) ||
                                        ( B*y + vectors_y[x][y] + i + l) < 0 ||
                                         (B*y + vectors_y[x][y] + i + l) > (M-1))
                                {
                                    q2 = 0;
                                } else {
                                    q2 = previous[ B*x + vectors_y[x][y]+k][ B*y + vectors_y[x][y]+i+l];
                                }
                                disty += abs(p1 - q2);
                            }
                            else {
