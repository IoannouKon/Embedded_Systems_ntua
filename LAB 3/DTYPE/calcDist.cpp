...
void calcDistancesHW(float* data_hw, float* dists_hw)
{
    ...
    #pragma HLS array_partition variable=data_hw_tmp block dim=2 factor=8
    #pragma HLS array_partition variable=data_hw_tmp block dim=1 factor=8
    #pragma HLS array_partition variable=movie_tmp block dim=1 factor=8
    #pragma HLS array_partition variable=dists_hw_tmp block dim=1 factor=8
    int i, j;
